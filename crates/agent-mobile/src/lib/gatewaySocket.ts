...
/**
 * Fires after the singleton client has been replaced (token or gateway origin
 * change). Module scoped stores that subscribed on the old instance re-attach
 * here — a disposed client never delivers events again.
 */
...
export function getGatewayWebSocketClient(token: string): GatewayWebSocketClient {
  ...
  if (activeClient && activeToken === normalizedToken && activeOrigin === origin) {
    return activeClient;
  }
